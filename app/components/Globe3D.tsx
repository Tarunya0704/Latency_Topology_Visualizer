...
function ExchangeMarker({ exchange, onClick, isSelected }: ExchangeMarkerProps) {
  ...
  const position = useMemo(() => LatLonToVector3(exchange.lat, exchange.lon, 2.05), [exchange.lat, exchange.lon])
    ...
    <group position={position}>
      ...
      {(hovered || isSelected) && (
        <Html distanceFactor={6} style={{ pointerEvents: "none" }}>
          <div
            className={`bg-black/95 text-white px-4 py-3 rounded-lg text-xs whitespace-nowrap border-2 ${
  ...
  return (
    <>
      ...
      <Html position={midPoint} distanceFactor={8} style={{ pointerEvents: "none" }}>
        <div className="bg-black/90 text-white px-3 py-1.5 rounded-md text-xs font-bold border border-gray-600 shadow-lg pointer-events-none">
...
function CloudRegionMarker({ region }: CloudRegionMarkerProps) {
  ...
  const position = useMemo(() => LatLonToVector3(region.lat, region.lon, 2.08), [region.lat, region.lon])
  ...
  return (
    <group position={position}>
      <mesh ref={ringRef} rotation={[Math.PI / 2, 0, 0]}>
        ...
        <meshBasicMaterial color={providerColors[region.provider]} transparent opacity={0.5} side={THREE.DoubleSide} />
      </mesh>

      <mesh rotation={[Math.PI / 2, 0, 0]}>
        <circleGeometry args={[0.06, 32]} />
        <meshBasicMaterial color={providerColors[region.provider]} transparent opacity={0.2} side={THREE.DoubleSide} />
      </mesh>

      <Html distanceFactor={12} style={{ pointerEvents: "none" }}>
        <div className="text-white text-[9px] bg-black/70 px-2 py-1 rounded whitespace-nowrap pointer-events-none">
...
export default function Globe3D({ exchanges, latencyData, cloudRegions }: Globe3DProps) {
  ...
  const filteredRegions = useMemo(() => {
    ...
    return cloudRegions.filter((region) => selectedCloudProvider === "all" || region.provider === selectedCloudProvider)
  ...
  return (
    ...
    >
      ...
      <HeatmapOverlay exchanges={filteredExchanges} latencyData={latencyData} visible={showRegions} />
